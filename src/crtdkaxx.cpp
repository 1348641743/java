...
static unsigned int sqlctx = 150203;
...
{12,4130,852,0,0,
5,0,0,1,75,0,4,5112,0,0,3,2,0,1,0,2,3,0,0,1,97,0,0,1,97,0,0,
};
...
namespace
{
 CDzczxx zczxx;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2637 "crtdkaxx.pc"

 char FRDM_ZCZXX[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 2639 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2641 "crtdkaxx.pc"
...
namespace
{
 CDdzkgx dzkgx;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2755 "crtdkaxx.pc"

 char FRDM_DZKGX[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 2757 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2759 "crtdkaxx.pc"
...
namespace
{
 CDdkhzx dkhzx;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2941 "crtdkaxx.pc"

 char FRDM_DKHZX[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 2943 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 2945 "crtdkaxx.pc"
...
namespace
{
 CDdkzdz dkzdz;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3079 "crtdkaxx.pc"

 char FRDM_DKZDZ[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 3081 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3083 "crtdkaxx.pc"
...
namespace
{
 CDzczjt zczjt;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3354 "crtdkaxx.pc"

 char FRDM_ZCZJT[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 3356 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3358 "crtdkaxx.pc"
...
namespace
{
 CDzczll zczll;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3551 "crtdkaxx.pc"

 char FRDM_ZCZLL[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 3553 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3555 "crtdkaxx.pc"
...
namespace
{
 CDzczpz zczpz;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3675 "crtdkaxx.pc"

 char FRDM_ZCZPZ[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 3677 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3679 "crtdkaxx.pc"
...
namespace
{
 CDmzhye mzhye;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3939 "crtdkaxx.pc"

 char FRDM_MZHYE[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 3941 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 3943 "crtdkaxx.pc"
...
namespace
{
 CDzzhmx zzhmx;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4106 "crtdkaxx.pc"

 char FRDM_ZZHMX[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4108 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4110 "crtdkaxx.pc"

typedef struct _TBL_DKAXX
{
  char frdm_u[4 +1];
  char khbh_u[12 +1];
  char kh_uuu[22 +1];
...
typedef struct _TBL_DKAXX TBL_DKAXX;
/* EXEC SQL END DECLARE SECTION; */ 
#line 4171 "crtdkaxx.pc"

typedef struct _DKAXX_UPDFLAG
{
 int FRDM_U_FLAG;
 int KHBH_U_FLAG;
 int KH_UUU_FLAG;
...
class CDdkaxx
{
 ...
 TBL_DKAXX preDKAXX;
 TBL_DKAXX pstDKAXX;
 TBL_DKAXX TBL_Array[100];
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4256 "crtdkaxx.pc"

  char dkaxx_FRDM_U[4 +1];
  char dkaxx_KHBH_U[12 +1];
  char dkaxx_KH_UUU[22 +1];
  char dkaxx_KLXBH_[10 +1];
  char dkaxx_HQYBTH[50 +1];
  char dkaxx_DQYBTH[50 +1];
  char dkaxx_YWZBZ_[10 +1];
  char dkaxx_KAXZ_U[1 +1];
  char dkaxx_KZL_UU[1 +1];
  char dkaxx_KAJZ_U[1 +1];
  char dkaxx_KDX_UU[1 +1];
  char dkaxx_ZH_UUU[50 +1];
  char dkaxx_XMPY_U[200 +1];
  char dkaxx_GRMC_U[200 +1];
  char dkaxx_KDJ_UU[1 +1];
  char dkaxx_ZFKBZ_[1 +1];
  char dkaxx_ZKKH_U[22 +1];
  char dkaxx_YYJG_U[9 +1];
  char dkaxx_YZSQRQ[8 +1];
  char dkaxx_ZKZXDM[9 +1];
  char dkaxx_FKJG_U[9 +1];
  char dkaxx_FAKGY_[10 +1];
  char dkaxx_ZCFKRQ[8 +1];
  char dkaxx_FAKARQ[8 +1];
  char dkaxx_FKFSDM[1 +1];
  char dkaxx_HXGY_U[10 +1];
  char dkaxx_HXRQ_U[8 +1];
  char dkaxx_HXJG_U[9 +1];
  char dkaxx_XLMMBZ[1 +1];
  char dkaxx_YXRQ_U[8 +1];
  char dkaxx_PZZT_U[1 +1];
  char dkaxx_SFYMMF[1 +1];
  char dkaxx_FKQD_U[2 +1];
  char dkaxx_FKLXR_[32 +1];
  char dkaxx_DFZWJG[9 +1];
  char dkaxx_SFJCCV[1 +1];
  char dkaxx_SFGNKZ[1 +1];
  char dkaxx_YXHRQ_[8 +1];
  char dkaxx_KBZ_UU[1 +1];
  char dkaxx_YGKBS_[1 +1];
  char dkaxx_SFZDXQ[1 +1];
  char dkaxx_SFZDXK[1 +1];
  char dkaxx_SFCSZD[1 +1];
  char dkaxx_GJKBZ_[1 +1];
  char dkaxx_YKTBZ_[1 +1];
  char dkaxx_XLKBZ_[1 +1];
  char dkaxx_SFSBCZ[1 +1];
  char dkaxx_SFGZK_[1 +1];
  char dkaxx_ZYDM_U[6 +1];
  char dkaxx_ZYMS_U[100 +1];
  char dkaxx_WHGY_U[10 +1];
  char dkaxx_WHJG_U[9 +1];
  char dkaxx_WHRQ_U[8 +1];
  long dkaxx_WHSJ_U;
  long dkaxx_SJC_UU;
  char dkaxx_JLZT_U[1 +1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4313 "crtdkaxx.pc"

 struct _DKAXX_UPDFLAG DKAXX_UPDFLAG;
 public:
  int DBProc();
  int INITA();
  int ROUND(TBL_DKAXX *pstBuf);
  int OPENCURSOR();
  int FETCHCURSOR();
  int CLOSECURSOR();
  int PRINTSQL();
  ...
  void LOG();
  void TRIM();
  void TRIMS(TBL_DKAXX *stTbl);
  void LOGS(TBL_DKAXX *stTbl);
  void INITD(TBL_DKAXX *stmpTbl);
 private:
  int INS();
  int SEL1();
  int SEL1L();
...
namespace
{
 CDdkaxx dkaxx;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4364 "crtdkaxx.pc"

 char FRDM_DKAXX[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4366 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4368 "crtdkaxx.pc"

typedef struct _TBL_DKPZF
{
  char frdm_u[4 +1];
  char kh_uuu[22 +1];
  long xhaos_;
...
typedef struct _TBL_DKPZF TBL_DKPZF;
/* EXEC SQL END DECLARE SECTION; */ 
#line 4404 "crtdkaxx.pc"

typedef struct _DKPZF_UPDFLAG
{
 int FRDM_U_FLAG;
 int KH_UUU_FLAG;
 int XHAOS__FLAG;
...
class CDdkpzf
{
 ...
 TBL_DKPZF preDKPZF;
 TBL_DKPZF pstDKPZF;
 TBL_DKPZF TBL_Array[100];
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4464 "crtdkaxx.pc"

  char dkpzf_FRDM_U[4 +1];
  char dkpzf_KH_UUU[22 +1];
  long dkpzf_XHAOS_;
  long dkpzf_JYCCCS;
  char dkpzf_HKJG_U[9 +1];
  long dkpzf_HKCS_U;
  double dkpzf_JF_UUU;
  double dkpzf_NLJSKE;
  long dkpzf_NLJSKS;
  char dkpzf_SFXFDJ[1 +1];
  long dkpzf_XFDJQX;
  long dkpzf_KDEXFH;
  long dkpzf_KDEQTH;
  long dkpzf_KXEXFH;
  ...
  long dkpzf_DQKHQK;
  long dkpzf_JJYJSY;
  char dkpzf_YULZID[10 +1];
  char dkpzf_SLBH_U[21 +1];
  char dkpzf_BZ_UUU[200 +1];
  char dkpzf_XMFH_U[20 +1];
  char dkpzf_XYSFRQ[8 +1];
  char dkpzf_WHGY_U[10 +1];
  char dkpzf_WHJG_U[9 +1];
  char dkpzf_WHRQ_U[8 +1];
  long dkpzf_WHSJ_U;
  long dkpzf_SJC_UU;
  char dkpzf_JLZT_U[1 +1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4496 "crtdkaxx.pc"

 struct _DKPZF_UPDFLAG DKPZF_UPDFLAG;
 public:
  int DBProc();
  int INITA();
  int ROUND(TBL_DKPZF *pstBuf);
  int OPENCURSOR();
  int FETCHCURSOR();
  int CLOSECURSOR();
  int PRINTSQL();
  ...
  void LOG();
  void TRIM();
  void TRIMS(TBL_DKPZF *stTbl);
  void LOGS(TBL_DKPZF *stTbl);
  void INITD(TBL_DKPZF *stmpTbl);
 private:
  int INS();
  int SEL1();
  int SEL1L();
...
namespace
{
 CDdkpzf dkpzf;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4533 "crtdkaxx.pc"

 char FRDM_DKPZF[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4535 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4537 "crtdkaxx.pc"
...
namespace
{
 CDdpzdj dpzdj;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4795 "crtdkaxx.pc"

 char FRDM_DPZDJ[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 4797 "crtdkaxx.pc"

};
/* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 4799 "crtdkaxx.pc"
...
namespace
{
 CDzczbc zczbc;
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 5014 "crtdkaxx.pc"

 char FRDM_ZCZBC[4 + 1];
 /* EXEC SQL END DECLARE SECTION; */ 
#line 5016 "crtdkaxx.pc"
...
int printDKAXX(TBL_DKAXX stDKAXX,char *sYYJG_U);
int printDKPZF(TBL_DKPZF stDKPZF,char *sYYJG_U);
int main(int argc, char **argv)
{
 ...
 char sFormat2[10];
 /* EXEC SQL BEGIN DECLARE SECTION; */ 
#line 5069 "crtdkaxx.pc"

  char sZHKHJG[9 +1]="";
   char sFRDM_U[4 +1]="";
  long inum = 0;
 /* EXEC SQL END DECLARE SECTION; */ 
#line 5073 "crtdkaxx.pc"
...
#line 5112 "crtdkaxx.pc"
 ...
 for(i = 0; i<atol(argv[3]); i++)
 {
  ...
  printf(" frdm_u1 = [%s] frdm_u2 = [%s]\n",stZCZXX.frdm_u, zczxx.stZCZXX.frdm_u);
  printf(" zhzt_u1 = [%s] zhzt_u2 = [%s]\n",stZCZXX.zhzt_u, zczxx.stZCZXX.zhzt_u);
  printZCZXX(stZCZXX,sZHKHJG);
  memset(&stDZKGX, 0, sizeof(stDZKGX));
  strcpy(stDZKGX.frdm_u, sFRDM_U);
  dzkgx.stDZKGX.khgxl1 = 0;
 ...
 memset(&stZCZPZ, 0, sizeof(stZCZPZ));
  ...
  printZCZPZ(stZCZPZ,sZHKHJG);
   memset(&stDKPZF, 0, sizeof(stDKPZF));
  ...
  memset(&dkaxx.stDKAXX, 0, sizeof(dkaxx.stDKAXX));
  strcpy(stDKAXX.frdm_u, argv[1]);
  strcpy(stDKAXX.khbh_u,sGLKHBH);
  sprintf(stDKAXX.kh_uuu,"%s%s%s", "622422",argv[2],sSXH_UU);
  strcpy(stDKAXX.klxbh_,"QXSBK020");
  strcpy(stDKAXX.hqybth, "");
  strcpy(stDKAXX.dqybth, "");
  strcpy(stDKAXX.ywzbz_, "1000000000");
  strcpy(stDKAXX.kaxz_u,"0");
  strcpy(stDKAXX.kzl_uu, "0");
  strcpy(stDKAXX.kajz_u,"0");
  strcpy(stDKAXX.kdx_uu, "0");
  strcpy(stDKAXX.zh_uuu, sKHZH_U);
  strcpy(stDKAXX.xmpy_u,"" );
  strcpy(stDKAXX.grmc_u, "");
  strcpy(stDKAXX.kdj_uu, "");
  strcpy(stDKAXX.zfkbz_, "0");
  sprintf(stDKAXX.zkkh_u,"%s%s%s", "622422",argv[2],sSXH_UU);
  strcpy(stDKAXX.yyjg_u, argv[2]);
  strcpy(stDKAXX.yzsqrq, "20190701");
  strcpy(stDKAXX.zkzxdm, "");
  strcpy(stDKAXX.fkjg_u, "");
  strcpy(stDKAXX.fakgy_, "009312");
  strcpy(stDKAXX.zcfkrq, "20110701");
  strcpy(stDKAXX.fakarq, "20110701");
  strcpy(stDKAXX.fkfsdm, "0");
  strcpy(stDKAXX.hxgy_u, "");
  strcpy(stDKAXX.hxrq_u, "");
  strcpy(stDKAXX.hxjg_u, "");
  strcpy(stDKAXX.xlmmbz, "2");
  strcpy(stDKAXX.yxrq_u, "20190701");
  strcpy(stDKAXX.pzzt_u, "0");
  strcpy(stDKAXX.sfymmf, "0");
  strcpy(stDKAXX.fkqd_u, "0");
  strcpy(stDKAXX.fklxr_, "li");
  strcpy(stDKAXX.dfzwjg, "0093");
  strcpy(stDKAXX.sfjccv, "1");
  strcpy(stDKAXX.sfgnkz, "0");
  strcpy(stDKAXX.yxhrq_, "");
  strcpy(stDKAXX.kbz_uu, "0");
  strcpy(stDKAXX.ygkbs_, "0");
  strcpy(stDKAXX.sfzdxq, "0");
  strcpy(stDKAXX.sfzdxk, "0");
  strcpy(stDKAXX.sfcszd, "");
  strcpy(stDKAXX.gjkbz_, "0");
  strcpy(stDKAXX.yktbz_, "2");
  strcpy(stDKAXX.xlkbz_, "");
  strcpy(stDKAXX.sfsbcz, "");
  strcpy(stDKAXX.sfgzk_, "");
  strcpy(stDKAXX.zydm_u, "3002");
  strcpy(stDKAXX.zyms_u, "实时单张个人主卡");
  strcpy(stDKAXX.whgy_u, "009312");
  strcpy(stDKAXX.whjg_u, "0093");
  strcpy(stDKAXX.whrq_u, "20110701");
  stDKAXX.whsj_u = 0;
  stDKAXX.sjc_uu = 0;
  strcpy(stDKAXX.jlzt_u, "0");
  printDKAXX(stDKAXX,sZHKHJG);
  memset(&dkpzf.stDKPZF, 0, sizeof(dkpzf.stDKPZF));
  strcpy(stDKPZF.frdm_u, argv[1]);
  sprintf(stDKPZF.kh_uuu, "%s%s%s","622422",argv[2],sSXH_UU);
  stDKPZF.jycccs = 0;
  strcpy(stDKPZF.hkjg_u, "");
  stDKPZF.hkcs_u = 0;
  stDKPZF.jf_uuu = 0;
  stDKPZF.nljske = 0;
  stDKPZF.nljsks = 0;
  strcpy(stDKPZF.sfxfdj, "");
  stDKPZF.xfdjqx = 0;
  stDKPZF.kdexfh = 0;
  stDKPZF.kdeqth = 0;
  stDKPZF.kxexfh = 0;
  ...
  stDKPZF.dqkhqk = 0;
  stDKPZF.jjyjsy = 0;
  strcpy(stDKPZF.yulzid, "1100000000");
  strcpy(stDKPZF.slbh_u, "622498660100012286");
  strcpy(stDKPZF.bz_uuu, "");
  strcpy(stDKPZF.xmfh_u, "");
  strcpy(stDKPZF.xysfrq, "0");
  strcpy(stDKPZF.whgy_u, "");
  strcpy(stDKPZF.whjg_u, "0050");
  strcpy(stDKPZF.whrq_u, "20110701");
  stDKPZF.whsj_u = 0;
  stDKPZF.sjc_uu = 0;
  strcpy(stDKPZF.jlzt_u, "0");
    printDKPZF(stDKPZF,sZHKHJG);
 }
 return 0;
}
